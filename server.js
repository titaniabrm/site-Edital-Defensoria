...
const subjectiveQuestions = [
  ...
  { text: "Explique o que é a vinculação de decisões e como ela se relaciona ao princípio da isonomia quando dois membros, envolvidos no mesmo episódio, recebem punições diferentes.", modelAnswer: "A vinculação de decisões garante que casos idênticos tenham soluções idênticas. Ligada à isonomia, impede que membros em situação igual recebam tratamento diferente, pois isso geraria injustiça e insegurança." },
  { text: "Segundo o Entendimento Vinculante sobre traição institucional, em quais hipóteses a traição NÃO se configura? Explique por que ocupar função administrativa em outra estrutura pode afastar essa excludente.", modelAnswer: "A traição não se configura quando, cumulativamente, o membro não divulga outra instituição, não participa de seu desenvolvimento e não ocupa função administrativa em outra. Ocupar cargo administrativo afasta a excludente porque demonstra envolvimento ativo na outra estrutura." },
  { text: "Um membro da Corregedoria pode atuar como defensor de alguém em processo que tramita contra a própria Corregedoria? Justifique com base no conflito de interesse e na imparcialidade.", modelAnswer: "Não pode. Configura conflito de interesse, pois fere a imparcialidade, a moralidade e a impessoalidade. O ideal é designar um defensor não impedido." },
  { text: "O que acontece com um processo disciplinar quando o réu sai voluntariamente ou é banido de todas as instituições? Explique o conceito de perda do objeto e o que deve ser requerido.", modelAnswer: "O processo perde o objeto, pois não há punição a aplicar a quem não integra mais a instituição. Deve-se requerer o arquivamento por perda superveniente do objeto, com possível reabertura caso a pessoa retorne." },
  { text: "Sobre quem recai o ônus de provar os fatos numa acusação? Explique por que um processo não pode ser mantido em aberto à espera de provas que ainda não foram apresentadas.", modelAnswer: "O ônus recai sobre quem acusa (parte autora). O processo não pode ficar em aberto esperando provas inexistentes, pois isso viola a presunção de inocência, a ampla defesa e a duração razoável do processo." }
...
async function notifyDiscord(submission, baseUrl) {
  ...
  await postWebhook({
    embeds: [{
      author: { name: "📥 Nova inscrição recebida" },
      title: `@${id.discord || "candidato"}`,
      url: target,
      description: "Um candidato enviou suas respostas e está aguardando avaliação da banca.",
      color: 0x4a6f8a,
      fields: [
...
async function notifyStatusChange(identity, status, baseUrl, submissionId) {
  ...
  const config = {
    Aprovado: { emoji: "✅", color: 0x287355, title: "Candidato aprovado", text: "Bem-vindo à Defensoria-Geral do Exército!" },
    Reprovado: { emoji: "❌", color: 0xb73d35, title: "Candidato reprovado", text: "Resultado divulgado ao candidato." },
    "Em analise": { emoji: "🔎", color: 0xb97518, title: "Voltou para análise", text: "A banca reabriu a avaliação deste candidato." }
  };
...
function requireSession(req, res, next) {
  const session = readSession(req);
  if (!session) {
    res.status(401).json({ error: "Faça login com Discord para continuar." });
    return;
  }
  req.session = session;
  next();
...
app.get("/api/health", async (req, res) => {
  const checks = { database: "local-json", databaseOk: true, groq: "não configurado", groqOk: null };
...
app.get("/api/admin/discord/start", (req, res) => {
  if (!DISCORD_CLIENT_ID || !DISCORD_REDIRECT_URI || !DISCORD_CLIENT_SECRET) {
    res.status(503).json({ error: "Login Discord não configurado no servidor." });
    return;
  }
...
app.post("/api/submissions", requireSession, rateLimitSubmissions, async (req, res) => {
  try {
    ...
    if (!captchaOk) {
      res.status(400).json({ error: "Captcha inválido ou expirado. Recarregue a página e tente de novo." });
      return;
    }
    ...
    if (duplicate) {
      res.status(409).json({
        error: "Já existe um envio registrado com este Discord ou Roblox.",
...
app.get("/api/admin/submissions", requireAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Erro ao listar envios. Verifique configuração do banco." });
  }
});
